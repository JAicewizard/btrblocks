...
// -------------------------------------------------------------------------------------
// example2.double:
// s3://public-bi-benchmark/binary/Telco/1/Telco_1/106_RECHRG_INC_MIN_USED_P1.double
// example2.bitmap:
// s3://public-bi-benchmark/binary/Telco/1/Telco_1/106_RECHRG_INC_MIN_USED_P1.bitmap
// -------------------------------------------------------------------------------------
...
bool test_compression(DoubleScheme& scheme,
                      DoubleStats& stats,
                      T* src,
                      size_t size,
                      PerfEvent& e,
  ...
  for (auto i = 0ul; i != size; ++i) {
    if (src[i] != dst[i]) {
      ...
      std::bitset<64> s(myUnion{src[i]}.iValue);
      std::bitset<64> d(myUnion{dst[i]}.iValue);
      std::cout << s << std::endl;
      std::cout << d << std::endl;
    }
    die_if(src[i] == dst[i]);
...
void setupSchemePool() {
  ...
  for (auto it = schemes.integer_schemes.begin(); it != schemes.integer_schemes.end();) {
    if (it->first != IntegerSchemeType::BP && it->first != IntegerSchemeType::UNCOMPRESSED) {
...
int main(int argc, char* argv[]) {
  ...
  PerfEvent perf;
  std::cerr << "using cascade depth " << FLAGS_cascade_depth << " and input file "
            << FLAGS_file_list_file << std::endl;
  ...
  while (file_list.next(nextfile)) {
    ...
    vector<double> doubles1;
    generate_n(std::back_inserter(doubles1), N, std::ref(generator));
    ...
    // }
    Vector<T> doubles(N);
    memcpy(doubles.data, doubles1.data(), N);
    doubles[0] =
        myUnion{.iValue = 0b0100000010010100100111011000000000000000000000000000000000000000}
            .dValue;
*/
    Vector<T> doubles(nextfile.c_str());
    ...
    perf.setParam("scheme", "decimal2");
    doubles::Decimal pd2;
    test_compression(pd2, stats, doubles.data, doubles.count, perf, 1);
    test_compression(pd2, stats, doubles.data, doubles.count, perf, 2);
