...
int main(int argc, char** argv) {
  ...
  // Create relation
  Relation relation =
      files::readDirectory(schema, FLAGS_binary.back() == '/' ? FLAGS_binary : FLAGS_binary + "/");
  ...
  for (SIZE col = 0; col < relation.columns.size(); col++) {
    ...
    stats_stream << relation.columns[col].name << "," << sizes_uncompressed[col] << ","
                 << sizes_compressed[col] << "\n";
  }
  auto end_time = std::chrono::steady_clock::now();
  uint64_t btr_creation_time =
      std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();
